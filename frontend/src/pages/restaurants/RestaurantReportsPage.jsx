...
import { Card, CardHeader, PageLoader, EmptyState } from '../../components/ui/index.jsx';
...
export default function RestaurantReportsPage() {
  ...
  const outlets = useMemo(() => [...new Set((data?.dailySales || []).map((d) => d.restaurant_name))], [data]);
  const daily = useMemo(() => {
    if (!data) return [];
    const days = [...new Set(data.dailySales.map((d) => d.day))];
    return days.map((day) => {
      const row = { day };
      outlets.forEach((name) => {
        row[name] = Number(data.dailySales.find((d) => d.day === day && d.restaurant_name === name)?.total || 0);
      });
      return row;
    });
  }, [data, outlets]);
  ...
  const ordersCount = data.dailySales.reduce((s, d) => s + Number(d.orders), 0);

  return (
    <div className="space-y-5">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-brand-600">Restaurants</p>
        <h1 className="text-2xl font-bold text-ink-900 mt-0.5">Reports</h1>
        <p className="text-sm text-ink-500 mt-1">Paid sales across every outlet.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="p-4"><p className="text-xs text-ink-500">Sales</p><p className="text-xl font-bold">{naira(totalRest)}</p></Card>
        <Card className="p-4"><p className="text-xs text-ink-500">Orders</p><p className="text-xl font-bold">{ordersCount}</p></Card>
        <Card className="p-4"><p className="text-xs text-ink-500">Top item</p><p className="text-xl font-bold truncate">{data.byItem[0]?.item_name || '—'}</p></Card>
        <Card className="p-4"><p className="text-xs text-ink-500">Top outlet</p><p className="text-xl font-bold truncate">{data.byRestaurant[0]?.name || '—'}</p></Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          <div className="p-4 grid gap-3">
            ...
            {data.byRestaurant.length === 0 && <EmptyState title="No sales yet" />}
          </div>
        </Card>
      ...
      <Card>
        ...
        <div className="p-4 h-72">
          {daily.length === 0 ? <EmptyState title="No paid orders yet" /> : (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={daily}>
                ...
                <Legend />
                {outlets.map((name, i) => (
                  <Bar key={name} dataKey={name} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                ))}
              </BarChart>
            </ResponsiveContainer>
          )}
      ...
      <Card>
        ...
        <div className="divide-y divide-ink-100">
          ...
          {data.byItem.length === 0 && <EmptyState title="Nothing sold yet" />}
        </div>
      </Card>
    </div>
  );
